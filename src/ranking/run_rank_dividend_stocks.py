# ===============================
# File: run_rank_dividend_stocks.py
# Purpose: Select and persist Top-K dividend names from ai_dividend_scores
# ===============================
...
MODEL_VERSION = os.getenv("DIVIDEND_MODEL_VERSION", datetime.now().strftime("v%Y%m%d"))
TOP_K = int(os.getenv("DIVIDEND_TOP_K", 20))
SCORE_TABLE = "ai_dividend_scores"
OUT_TABLE = "ai_dividend_portfolio"
SCORE_TYPE = "dividend"
...
def main():
    ...
    if df.empty:
        print("⚠️ No dividend scores for model_version", MODEL_VERSION)
        return
    ...
    print(f"✅ Saved Top-{len(top)} dividend picks for {as_of.date()} → {OUT_TABLE}")
